...
def test_compress_skipped_logic(client: typing.Any) -> None:
    from llmzip.pricing.fallback import FALLBACK_META, FALLBACK_PRICES

    with (
        patch("llmzip.api.routes.compress.count_tokens", return_value=(10, "exact")),
        patch(
            "llmzip.core.savings_calculator.resolve_prices",
            return_value=(FALLBACK_PRICES, FALLBACK_META),
        ),
    ):
        res = client.post("/v1/compress", json={"text": "short", "model": "gpt-4o", "ratio": 0.5})
...
def test_batch_schema_consistency(client: typing.Any) -> None:
    with (
        patch("llmzip.api.routes.compress.count_tokens", return_value=(10, "exact")),
        patch("llmzip.api.routes.compress.calculate_savings") as mock_calc,
    ):
        # Manually construct a savings result using fallback data
        mock_calc.return_value = MagicMock(
            estimated_savings={"claude-opus-4-8": "$0.01"}, pricing_note="fallback"
        )

        res = client.post(
            "/v1/compress/batch", json={"texts": [{"text": "short", "model": "claude-opus-4-8"}]}
        )
        assert res.status_code == 200
        data = res.json()
...
def test_batch_error_handling(client: typing.Any) -> None:
    with patch("llmzip.api.routes.compress.count_tokens", return_value=(1000, "exact")):
        ...
        res = client.post(
            "/v1/compress/batch",
            json={"texts": [{"text": "long text enough to trigger compression"}]},
        )
        data = res.json()
...
def test_estimate_endpoint(client: typing.Any) -> None:
    with patch("llmzip.api.routes.estimate.count_tokens", return_value=(100, "exact")):
        ...
        res = client.post(
            "/v1/estimate", json={"text": "medium text", "model": "gpt-4o", "ratio": 0.5}
        )
        assert res.status_code == 200
        data = res.json()
...
def test_compress_file_mocked(
    mock_count: typing.Any, mock_convert: typing.Any, client: typing.Any
) -> None:  # type: ignore
